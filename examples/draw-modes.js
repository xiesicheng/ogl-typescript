...
`
    ...
    resize();
    const scene = new Transform(); // Geometry is an indexed square, comprised of 4 vertices.
    ...
    points.position.set(-1, 1, 0); // gl.LINES: draws 3 lines - a line between each pair of vertices.
    // Ideal use for separated lines.

    const lineStrip = new Mesh(gl, {
      mode: gl.LINES,
      geometry,
      program
    });
    lineStrip.setParent(scene);
    lineStrip.position.set(1, 1, 0); // gl.LINE_LOOP: draws 6 lines (1 unavoidable overlap for squares).

    const lineLoop = new Mesh(gl, {
      mode: gl.LINE_LOOP,
      geometry,
      program
    });
    lineLoop.setParent(scene);
    lineLoop.position.set(-1, -1, 0); // gl.TRIANGLES: draws a triangle between each set of 3 vertices.
    // Used as the default draw mode, so doesn't really need to be passed in as a param.

    const triangles = new Mesh(gl, {
      mode: gl.TRIANGLES,
      geometry,
      program
    });
    triangles.setParent(scene);
    triangles.position.set(1, -1, 0); // OTHER MODES NOT FEATURED:
