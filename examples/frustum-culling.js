...
`
    ...
    function addCameraShape() {
      ...
      mesh.setParent(frustumTransform); // mesh.isCameraShape = true;
    ...
    function update(t) {
      requestAnimationFrame(update);
      controls.update(); // Move camera around a path
      ...
      frustumTransform.rotation.copy(frustumCamera.rotation); // Traverse all meshes in the scene

      scene.traverse(node => {
        if (!node.draw) return; // if (node.isCameraShape) return;

        if (node === cameraShape) return; // perform the frustum test using the demo camera
