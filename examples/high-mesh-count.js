...
`
    ...
    resize();
    const scene = new Transform(); // Create base geometry

    const cubeGeometry = new Box(gl); // Using the shader from the base primitive example
    ...
    function setMeshCount(count) {
      // sanitize input
      count = parseInt(count) || 1000; // remove old meshes

      for (let i = 0; i < meshes.length; ++i) scene.removeChild(meshes[i]);

      meshes = []; // create our meshes according to input
