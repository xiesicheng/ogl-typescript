...
`
  ...
  function updateMouse(e) {
    ...
    mouse.set(e.x / gl.renderer.width, 1.0 - e.y / gl.renderer.height); // Calculate velocity
    ...
    velocity.y = deltaY / delta; // Flag update to prevent hanging velocity values when not moving
  ...
  function update(t) {
    requestAnimationFrame(update); // Reset velocity when mouse not moving
    ...
    velocity.needsUpdate = false; // Update flowmap inputs

    flowmap.aspect = aspect;
    flowmap.mouse.copy(mouse); // Ease velocity input, slower when fading out
