...
`
    ...
    document.body.appendChild(gl.canvas); // gl.clearColor(1, 1, 1, 1);
    ...
    resize();
    const scene = new Transform(); // Scene to render to targets
    ...
    async function initScene() {
      const data = await (await fetch(`assets/acorn.json`)).json(); // Instanced buffers
    ...
    function initPost() {
      ...
      const program = new Program(gl, {
        ...
        uniforms: {
          tColor: {
            value: target.textures[0]
          },
          tPosition: {
            value: target.textures[1]
          },
          uLights: {
            value: lights
          },
    ...
    function update(t) {
      requestAnimationFrame(update);
      controls.update(); // Render scene to targets
      ...
      post.program.uniforms.uTime.value = t * 0.001; // Render post to canvas
