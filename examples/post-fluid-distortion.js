...
`
  ...
  resize(); // Helper functions for larger device support
  ...
  const simRes = 128;
  const dyeRes = 512; // Main inputs to control look and feel of fluid
  ...
  const radius = 0.2; // Common uniform

  const texelSize = {
    value: new Vec2(1 / simRes)
  ...
  const advectionProgram = new Mesh(gl, {
    ...
    program: new Program(gl, {
      ...
      uniforms: {
        texelSize,
        dyeTexelSize: {
          value: new Vec2(1 / dyeRes, 1 / dyeRes)
        },
        uVelocity: {
          value: null
        },
  ...
  const splats = []; // Create handlers to get mouse position and velocity
  ...
  function updateMouse(e) {
    ...
    if (!lastMouse.isInit) {
      lastMouse.isInit = true; // First input
    ...
    lastMouse.set(e.x, e.y); // Add if the mouse is moving
  ...
  function update(t) {
    requestAnimationFrame(update); // Perform all of the fluid simulation renders
    // No need to clear during sim, saving a number of GL calls.

    gl.renderer.autoClear = false; // Render all of the inputs since last frame
    ...
    density.swap(); // Set clear back to default

    gl.renderer.autoClear = true; // Update post pass uniform with the simulation output

    pass.uniforms.tFluid.value = density.read.texture;
    mesh.rotation.y -= 0.0025;
    mesh.rotation.x -= 0.005;
    pass.uniforms.uTime.value = t * 0.001; // Replace Renderer.render with post.render. Use the same arguments.
