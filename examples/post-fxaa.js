...
`
  ...
  camera.lookAt([0, 0, 0]); // Post copies the current renderer values (width, height, dpr) if none are passed in

  const post = new Post(gl); // Create uniform for pass
  ...
  function update() {
    ...
    mesh.rotation.x -= 0.01; // Replace Renderer.render with post.render. Use the same arguments.
