...
`
    ...
    cube.setParent(scene); // assign update functions to each mesh so they can share a program but
    // still have unique uniforms by updating them just before being drawn

    function updateHitUniform({
      mesh
    }) {
    ...
    const mouse = new Vec2(); // Create a raycast object

    const raycast = new Raycast(gl); // Define an array of the meshes we want to test our ray against

    const meshes = [plane, sphere, cube]; // By default, raycast.intersectBounds() tests against the bounding box.
    ...
    function move(e) {
      mouse.set(2.0 * (e.x / renderer.width) - 1.0, 2.0 * (1.0 - e.y / renderer.height) - 1.0); // Update the ray's origin and direction using the camera and mouse

      raycast.castMouse(camera, mouse); // Just for the feedback in this example - reset each mesh's hit to false

      meshes.forEach(mesh => mesh.isHit = false); // raycast.intersectBounds will test against the bounds of each mesh, and 
      // return an array of intersected meshes in order of closest to farthest

      const hits = raycast.intersectBounds(meshes); // Can intersect with geometry if the bounds aren't enough, or if you need
