...
`
    ...
    camera.lookAt([0, 0, 0]);
    const targetCamera = new Camera(gl, {
      fov: 35
    });
    ...
    function resize() {
      renderer.setSize(window.innerWidth, window.innerHeight); // Only update aspect of target camera, as first scene will be drawn to a square render target
    ...
    resize();
    const geometry = new Box(gl); // A little data texture with 4 colors just to keep things interesting
    ...
    function update() {
      ...
      targetMesh.rotation.x -= 0.01; // Set background for first render to target

      gl.clearColor(0.15, 0.05, 0.2, 1); // Add target property to render call
      ...
      gl.clearColor(1, 1, 1, 1); // Omit target to render to canvas
