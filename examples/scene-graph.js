...
`
    ...
    gl.clearColor(1, 1, 1, 1); // The Camera class extends Transform. See Below for more on Transform.
    ...
    const scene = new Transform(); // The Mesh class extends the Transform class, and so shares the scene graph functionality

    const sphere = new Mesh(gl, {
      geometry: sphereGeometry,
      program
    });
    sphere.speed = -0.5; // Use .setParent to add transform as a child of another transform

    sphere.setParent(scene); // scene.addChild(sphere); // also works

    const shapes = [sphere]; // Create random array of shapes

    for (let i = 0; i < 50; i++) {
      const geometry = Math.random() > 0.5 ? cubeGeometry : sphereGeometry;
      const shape = new Mesh(gl, {
        geometry,
        program
      });
      shape.scale.set(Math.random() * 0.3 + 0.7);
      shape.position.set((Math.random() - 0.5) * 3, (Math.random() - 0.5) * 3, (Math.random() - 0.5) * 3);
      shape.speed = (Math.random() - 0.5) * 0.7; // Attach them to a random, previously created shape
