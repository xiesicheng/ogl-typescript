...
`
    ...
    resize();
    const scene = new Transform(); // Swap between the 'fov' and 'left/right/etc' lines to switch from an orthographic to perspective camera,
    ...
    light.lookAt([0, 0, 0]); // Create shadow instance attached to light camera
    ...
    async function addAirplane() {
      ...
      mesh.setParent(scene); // Use the 'add' method to attach the mesh to the shadow map
    ...
    function update(t) {
      requestAnimationFrame(update);
      controls.update(); // A bit of plane animation
