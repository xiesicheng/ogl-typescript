...
`
    ...
    async function loadModel() {
      const data = await (await fetch(`assets/snout-rig.json`)).json(); // Rig JSON data format
      ...
      const animationData = await (await fetch(`assets/snout-anim.json`)).json(); // Animation JSON format
      ...
      const geometry = new Geometry(gl, {
        ...
        skinIndex: {
          size: 4,
          data: new Float32Array(data.skinIndex)
        },
        skinWeight: {
          size: 4,
          data: new Float32Array(data.skinWeight)
        }
      });
      ...
      skin.scale.set(0.01);
      skin.position.y = -1; // Helper function to add animation to skin's bones. 
    ...
    function update(t) {
      requestAnimationFrame(update); // Control animation but updating the elapsed value.
      // It uses modulo to repeat the animation range, 
      // so below is playing a never-ending loop.

      if (animation) animation.elapsed += 0.1; // Calling 'update' updates the bones with all of the
