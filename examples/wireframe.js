...
`
    ...
    async function loadModel() {
      const data = await (await fetch(`assets/croissant.json`)).json(); // Regular geometry with no modifications to be used with gl.LINE_LOOP version
      ...
      lineLoopMesh = new Mesh(gl, {
        mode: gl.LINE_LOOP,
        geometry,
        program
      });
      lineLoopMesh.setParent(scene);
      lineLoopMesh.position.y = 1; // For an accurate wireframe, triangle vertices need to be duplicated to make line pairs.
